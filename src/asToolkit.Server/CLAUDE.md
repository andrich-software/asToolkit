# CLAUDE.md — asToolkit.Server

ASP.NET Core Web API (net10.0). Headless backend; consumed by `asToolkit.Client` and any HTTP client.
...
Rules:
- **Primary-constructor injection** of `IMediator` (the custom one from `asToolkit.Application.Mediator`).
...
## CQRS in asToolkit.Application

Commands and queries live in `src/asToolkit.Application/Features/{Area}/{Commands|Queries}/{Name}/`. Each folder typically contains:
...
The Mediator is **custom** — `asToolkit.Application.Mediator.CustomMediator` resolves `IRequestHandler<,>` via DI and dispatches by reflection. Do not add the `MediatR` NuGet package; some legacy doc comments mention it but the implementation is in-house.

Returns are wrapped in `Result<T>` / `PaginatedResult<T>` (`asToolkit.Domain.Wrapper`). The extension `response.ToActionResult()` maps `Result` to the right HTTP status with RFC 7807 problem details on failures.
...
| Task | Where |
|---|---|
| Add a CRUD resource | New `Features/{Area}/...` folder + `{Area}Controller.cs` + DTOs in `asToolkit.Domain/Dtos/{Area}` |
| New auth role | `asToolkit.Identity` constants + role check in handler + tests |
...
Server integration tests live in `tests/asToolkit.Server.Tests` (xUnit). All multi-tenant tests inherit `TenantIsolatedTestBase`. See that project's `CLAUDE.md`.
