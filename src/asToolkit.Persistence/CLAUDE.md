# CLAUDE.md — asToolkit.Persistence

EF Core data access layer. Provider-neutral; the four concrete providers live in sibling projects:

- `asToolkit.Persistence.MSSQL`
- `asToolkit.Persistence.PostgreSQL`
- `asToolkit.Persistence.SQLite`
...
Zero-based pagination, implemented in `QueryableExtensions.cs` (in `asToolkit.Application` or this layer — see the file). New endpoints **must** use it; do not roll your own.
...
Persistence-level tests live in `tests/asToolkit.Persistence.Tests` (xUnit, EF Core InMemory). Integration tests that hit the API surface live in `tests/asToolkit.Server.Tests` — see its `CLAUDE.md`.
