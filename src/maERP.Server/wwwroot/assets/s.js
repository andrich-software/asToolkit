/*
 * maERP storefront tracker (canonical core).
 *
 * Privacy & security model:
 *  - Sends beacons to a SAME-ORIGIN collector path on the shop (configured by the plugin). The shop
 *    plugin proxies them server-side to maERP, adding the secret token + visitor IP + (when logged in)
 ...
 *
 * Configure before loading by setting window.maerpTrackConfig = { endpoint: '/same-origin/path',
 * product: { ref, name, value, currency } }. The plugin injects this.
 */
(function () {
  "use strict";

  var cfg = window.maerpTrackConfig || {};
  var endpoint = cfg.endpoint;
  if (!endpoint) { return; }

  var VID_KEY = "maerp_vid";
