...
namespace asToolkit.Server.Tests.Features.Setting.Queries;
...
public class SettingListQueryTests : GlobalTestBase
{
    ...
    private async Task SeedSettingTestDataAsync()
    {
        ...
        if (!existingSettings.Any(s => s.Key == "test.setting.global"))
        {
            ...
            var setting3 = new asToolkit.Domain.Entities.Setting
            {
                Key = "test.theme.global",
                Value = "dark"
            };

            var setting4 = new asToolkit.Domain.Entities.Setting
            {
                Key = "test.config.global",
                Value = "config_value"
            };

            var setting5 = new asToolkit.Domain.Entities.Setting
            {
