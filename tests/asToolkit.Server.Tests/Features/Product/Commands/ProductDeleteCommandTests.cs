...
public class ProductDeleteCommandTests : TenantIsolatedTestBase
{
    ...
    private async Task<(Guid tenant1ProductId, Guid tenant2ProductId)> SeedTestDataAsync()
    {
        ...
        try
        {
            ...
            if (!hasData)
            {
                ...
                var taxClass2 = new asToolkit.Domain.Entities.TaxClass
                {
                    Id = TaxClass2Id,
                    TaxRate = 20.0,
                ...
                var manufacturer = new asToolkit.Domain.Entities.Manufacturer
                {
                    Id = Manufacturer1Id,
                ...
                var manufacturer2 = new asToolkit.Domain.Entities.Manufacturer
                {
                    Id = Manufacturer2Id,
                ...
                var product3 = new asToolkit.Domain.Entities.Product
                {
                    Id = Product3Id,
                    Sku = "TEST-DELETE-003",
    ...
    {
        var dto = new asToolkit.Domain.Dtos.ProductAttribute.ProductAttributeInputDto
        {
            Name = name,
            SortOrder = 0,
            Values = values.Select((v, i) => new asToolkit.Domain.Dtos.ProductAttribute.ProductAttributeValueInputDto
            {
                Value = v,
                SortOrder = i
            }).ToList()
        ...
        var attrDetail = await ReadResponseAsync<Result<asToolkit.Domain.Dtos.ProductAttribute.ProductAttributeDetailDto>>(detailResponse);
    ...
    public async Task DeleteProduct_VariantParent_ShouldCascadeDeleteVariants()
    {
        ...
        var parentDto = new asToolkit.Domain.Dtos.Product.ProductInputDto
        {
            Sku = $"CASC-VP-{Guid.NewGuid():N}",
            Name = "Cascade Parent",
            Price = 0m,
            TaxClassId = TaxClass1Id,
            ProductType = asToolkit.Domain.Enums.ProductType.VariantParent,
        ...
        for (var i = 0; i < 2; i++)
        {
            var variantDto = new asToolkit.Domain.Dtos.Product.ProductInputDto
            {
                Sku = $"CASC-VAR-{i}-{Guid.NewGuid():N}",
                Name = $"Cascade Variant {i}",
                Price = 5m,
                TaxClassId = TaxClass1Id,
                ProductType = asToolkit.Domain.Enums.ProductType.Variant,
