...
namespace asToolkit.Server.Tests.Features.TaxClass.Commands;
...
public class TaxClassDeleteCommandTests : TenantIsolatedTestBase
{
    ...
    public async Task DeleteTaxClass_FromDifferentTenant_ShouldReturnNotFound()
    {
        ...
        var result = await ReadResponseAsync<Result<asToolkit.Domain.Dtos.TaxClass.TaxClassDetailDto>>(getResponse);
    ...
    public async Task DeleteTaxClass_AfterTenantSwitch_ShouldDeleteFromCorrectTenant()
    {
        ...
        var result = await ReadResponseAsync<Result<asToolkit.Domain.Dtos.TaxClass.TaxClassDetailDto>>(stillExists);
