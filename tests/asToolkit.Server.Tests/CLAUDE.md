# CLAUDE.md — asToolkit.Server.Tests

xUnit integration tests against `asToolkit.Server`, with first-class multi-tenant support.
...
Location: `tests/asToolkit.Server.Tests/Infrastructure/TenantIsolatedTestBase.cs`.
...
Location: `tests/asToolkit.Server.Tests/Infrastructure/TestDataSeeder.cs`. Idempotent. Seeds:
...
Location: `tests/asToolkit.Server.Tests/Infrastructure/TestAssertions.cs`. Thin wrappers around xUnit for common needs (HTTP status, nullability, collections).
