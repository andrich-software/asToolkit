...
{
    const OPT_SERVER_URL = 'maerp_analytics_server_url';
    const OPT_TOKEN      = 'maerp_analytics_token';

    const REST_NAMESPACE = 'maerp/v1';
    const REST_ROUTE     = '/c'; // first-party collector: /wp-json/maerp/v1/c
    ...
    {
        add_options_page(
            __('maERP Web Analytics', 'maerp-analytics'),
            __('maERP Analytics', 'maerp-analytics'),
            'manage_options',
            'maerp-analytics',
    ...
    {
        register_setting('maerp_analytics', self::OPT_SERVER_URL, [
            'type'              => 'string',
            'sanitize_callback' => [__CLASS__, 'sanitize_url'],
            'default'           => '',
        ]);
        register_setting('maerp_analytics', self::OPT_TOKEN, [
            'type'              => 'string',
    ...
    public static function render_settings_page()
    {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('maERP Web Analytics', 'maerp-analytics'); ?></h1>
            <p><?php esc_html_e('Connect this store to your maERP server. Generate a tracking token in maERP (Sales channel → Web analytics) and paste it here.', 'maerp-analytics'); ?></p>
            <form action="options.php" method="post">
                <?php settings_fields('maerp_analytics'); ?>
                <table class="form-table" role="presentation">
                    <tr>
                        <th scope="row"><label for="maerp_url"><?php esc_html_e('maERP Server URL', 'maerp-analytics'); ?></label></th>
                        <td>
                            <input name="<?php echo esc_attr(self::OPT_SERVER_URL); ?>" id="maerp_url" type="url"
                                   class="regular-text" placeholder="https://erp.example.com"
                                   value="<?php echo esc_attr(get_option(self::OPT_SERVER_URL, '')); ?>" />
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="maerp_token"><?php esc_html_e('Tracking Token', 'maerp-analytics'); ?></label></th>
                        <td>
                            <input name="<?php echo esc_attr(self::OPT_TOKEN); ?>" id="maerp_token" type="password"
                                   class="regular-text" autocomplete="off"
                                   value="<?php echo esc_attr(get_option(self::OPT_TOKEN, '')); ?>" />
                            <p class="description"><?php esc_html_e('Stored on the server only — never sent to the browser.', 'maerp-analytics'); ?></p>
                        </td>
                    </tr>
                </table>
                <?php submit_button(); ?>
    ...
    public static function enqueue_tracker()
    {
        ...
        $version = '1.0.0';
        $core_url = plugins_url('assets/maerp-s.js', __FILE__);
        $glue_url = plugins_url('assets/maerp-wc.js', __FILE__);

        wp_register_script('maerp-analytics-core', $core_url, [], $version, true);
        wp_register_script('maerp-analytics-wc', $glue_url, ['maerp-analytics-core', 'jquery'], $version, true);
        ...
        wp_localize_script('maerp-analytics-core', 'maerpTrackConfig', $config);
        wp_enqueue_script('maerp-analytics-core');
        wp_enqueue_script('maerp-analytics-wc');
    }

    // -------------------------------------------------------------------------
    // Same-origin collector → server-side proxy to maERP
    // -------------------------------------------------------------------------
