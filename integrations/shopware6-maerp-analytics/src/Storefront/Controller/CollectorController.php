...
/**
 * Same-origin collector that proxies storefront beacons to maERP. The secret tracking token is read from
 * the plugin config and added here — it never reaches the browser. When a customer is logged in, a
 * pseudonymised reference (cid) is computed and forwarded so maERP can attribute product views.
 */
...
class CollectorController extends StorefrontController
{
    ...
    #[Route(
        path: '/maerp-analytics/c',
        name: 'frontend.maerp.collect',
        methods: ['POST'],
    ...
    /**
     * Fire-and-forget forward to maERP with a tight timeout, so the beacon response is never blocked on
     * the upstream call.
     */
